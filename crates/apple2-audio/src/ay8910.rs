...
impl Ay8910 {
    ...
    fn envelope_volume(&self) -> u8 {
        let step = if self.env_holding {
            if self.env_hold {
                if self.env_alternate ^ self.env_attack {
                    15
                } else {
                    0
                }
    ...
    pub fn render(&mut self, out: &mut [f32], _clock_rate: f64, _sample_rate: u32) {
        ...
        let tone_enabled = [mixer & 0x01 == 0, mixer & 0x02 == 0, mixer & 0x04 == 0];
        let noise_enabled = [mixer & 0x08 == 0, mixer & 0x10 == 0, mixer & 0x20 == 0];
        ...
        for sample in out.iter_mut() {
            ...
            for ch in 0..NUM_CHANNELS {
                let gate =
                    (tone_enabled[ch] && self.tone_states[ch]) || (noise_enabled[ch] && noise_out);
                if gate {
