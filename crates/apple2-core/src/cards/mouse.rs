...
impl Card for MouseCard {
    fn card_type(&self) -> CardType { CardType::MouseInterface }
    ...
    fn slot_io_read(&mut self, reg: u8, _cycles: u64) -> u8 {
        ...
        match reg & 0x03 {
            0x0 => if self.cra & 0x04 != 0 { self.pra } else { self.ddra },
            0x1 => self.cra,
            0x2 => if self.crb & 0x04 != 0 { self.prb } else { self.ddrb },
            0x3 => self.crb,
            _ => 0xFF,
        }
    }
    ...
    fn slot_io_write(&mut self, reg: u8, val: u8, _cycles: u64) {
        match reg & 0x03 {
            0x0 => { if self.cra & 0x04 != 0 { self.pra = val; } else { self.ddra = val; } }
            0x1 => self.cra = val & 0x3F,
            0x2 => { if self.crb & 0x04 != 0 { self.prb = val; } else { self.ddrb = val; } }
    ...
    fn reset(&mut self, _power_cycle: bool) {
        self.pra = 0; self.prb = 0;
        self.ddra = 0; self.ddrb = 0;
