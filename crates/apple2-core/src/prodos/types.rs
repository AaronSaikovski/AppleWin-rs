...
pub fn copy_upper(dst: &mut [u8], src: &str) -> usize {
    ...
    for (i, b) in src.bytes().enumerate() {
        if i >= MAX_FILENAME { break; }
        let c = if b.is_ascii_lowercase() { b - b'a' + b'A' } else { b };
        dst[i] = c;
        n += 1;
    }
...
pub fn set_volume_header(image: &mut [u8], h: &VolumeHeader, block: u32) {
    ...
    for i in 0..(h.len as usize) { image[base + 1 + i] = h.name[i]; }
    for i in 0..8 { image[base + 16 + i] = h.pad8[i]; }
...
pub fn put_file_header(image: &mut [u8], off: usize, f: &FileHeader) {
    ...
    for i in 0..(f.len as usize) { image[off + 1 + i] = f.name[i]; }
